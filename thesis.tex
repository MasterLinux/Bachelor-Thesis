...
When Java sources files are generated from a diagram, they will be opened in blue accented editor tabs, which signalize that their included documents aren't saved yet and they are a result of the code generation. Furthermore the generated sources are including some special JavaDoc tags \ref{single_source}, which allow generating the identical diagram, after a restart of AlphaUML. The whole process with all necessary status information will be shown in the console tab, so the user gets an overview about the running process. 

\paragraph{Reverse Engineering}
In another case if a class diagram has to be generated, a parser parses each Java file and generates it with the help of the grammatical logic of the Java source. So it's necessary to keep with the correct Java grammar, otherwise the parser throws an error and skips the current parsed file.
