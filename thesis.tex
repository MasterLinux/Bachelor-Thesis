\documentclass[fontsize=12pt,a4paper,titlepage]{scrartcl}
...
\subtitle{Bachelor Thesis \\ University of Applied Sciences D\"{u}sseldorf \\ Department of Media}
\date{10.08.2011}
\author{Degree Program: Media and applied \\ Information Technology(B.Sc.) \\ Christoph Grundmann (515440)}
\publishers{First examiner: Prof. Dr.-Ing. MSc Markus Dahm \\ Second examiner: Prof. Dr. Christoph Thiel}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{images/alphauml.png}
\end{figure}
...
When a new project has to be created, the currently opened project is closed and an empty project without any Java file is created. This new project has the typical folder structure, which is described in \ref{folder_structure}. To create a new project the start menu entry "New Project" is used.

\subsubsection{Load Existing Project}
To open an existing project the "Open Project" entry of the start menu is used. It opens a specific folder which can be one without the usual folder structure of AlphaUML. If the folder contains a "src" folder, AlphaUML uses it as main folder for Java source files, otherwise the user has to declare a folder himself.
...
\begin{itemize}
	\item{Prototypal Inheritance \footnote{http://javascript.crockford.com/prototypal.html}}
	\item{Classical Inheritance \footnote{http://javascript.crockford.com/inheritance.html}}
	\item{Private Members \footnote{http://javascript.crockford.com/private.html}}
\end{itemize}
...
\begin{lstlisting}[caption={HTML snippet}, frame=single]
<div class="outer">
	<div class="inner"></div>
</div>
\end{lstlisting}

\begin{lstlisting}[caption={CSS snippet}, frame=single, label={css_example_snippet}]
.outer {
    width: 45px;
    height: 60px;
    margin: 10px;
...
\begin{figure}[H]
\centering
\includegraphics[scale=1]{images/css_example.png}
\caption{result of the css formatting}
\end{figure}
...
The Adobe Air HTML- and JavaScript engine is based on WebKit, an open source web browser engine. So it's necessary to know that some CSS features like text- or box-shadows need the prefix "-webkit-". The usage will be shown in the example above -- Listing \ref{css_example_snippet}.
...
\begin{figure}[H]
\centering
\includegraphics[scale=1]{images/window_structure.png}
\caption{logical structure of a window}
\end{figure}
...
If an interface is extended, a generalization \ref{generalization} has to be used. That includes the UML generalization arrow and the use of the "extends" keyword on the Java-side.
...
\paragraph{Basics of Compiler Design - Anniversary edition}
Torben \AE gidius Mogensen \\ http://www.diku.dk/hjemmesider/ansatte/torbenm/Basics/

\paragraph{Compiler. Prinzipien, Techniken und Tools}
Pearson Studium \\ ISBN-13: 978-3827370976

\paragraph{The Java$^{TM}$ Language Specification - Third Edition}
Addison Wesley \\ http://java.sun.com/docs/books/jls/

\paragraph{UML 2 - Das umfassende Handbuch - 3. Auflage} 
Galileo Computing \\ ISBN: 978-3-8362-1419-3

\paragraph{Pro JavaScript$^{TM}$ Techniques}
John Resig \\ ISBN: 1-59059-727-3
