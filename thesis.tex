...
\label{javac}
To compile the current opened project, it's possible to execute the "javac" command via the command line of the console. Currently it saves a generated class file of an associated Java File in same folder in which the Java file is placed. Moreover when the compiler throws an error, the console displays it. \newline To be able to execute the "javac" command the console needs the path of the java compiler, which has to be set in the AlphaUML options \ref{alphauml_options}.
